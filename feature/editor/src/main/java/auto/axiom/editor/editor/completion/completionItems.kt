...
abstract class AxiomEditorCompletionItem(
...
) : AxiomEditorCompletionItem(completionKind, label, desc) {
...
class AICompletionItem(
    ...
    desc: CharSequence?,
    private val commitText: String,
) : AxiomEditorCompletionItem(CompletionItemKind.AI_GENERATED, label, desc) {
    override fun performCompletion(editor: CodeEditor, text: Content, line: Int, column: Int) {
        text.insert(line, column, commitText)
    }
}
...
) : AxiomEditorCompletionItem(CompletionItemKind.SNIPPET, label, desc) {

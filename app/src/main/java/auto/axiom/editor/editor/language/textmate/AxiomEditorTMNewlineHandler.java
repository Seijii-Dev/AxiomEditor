...
package auto.axiom.editor.editor.language.textmate;
...
public class AxiomEditorTMNewlineHandler implements NewlineHandler {
  private final AxiomEditorTMLanguage language;
  ...
  public AxiomEditorTMNewlineHandler(AxiomEditorTMLanguage language) {
  ...
  protected Pair<String, String> getIndentForEnter(Content text, CharPosition position) {
    ...
    var afterEnterAction = getInheritIndentForLine(new AxiomEditorTMNewlineHandler.WrapperContentImp(text, position.line, beforeEnterText), true, position.line + 1);
   ...
   */
  @Nullable
  private AxiomEditorTMNewlineHandler.InheritIndentResult getInheritIndentForLine(AxiomEditorTMNewlineHandler.WrapperContent wrapperContent,
    ...
    if (line <= 0) {
      return new AxiomEditorTMNewlineHandler.InheritIndentResult("", null);
    }
    ...
    if (indentRulesSupport.shouldIncrease(precedingUnIgnoredLineContent) || indentRulesSupport.shouldIndentNextLine(precedingUnIgnoredLineContent)) {
      return new AxiomEditorTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(precedingUnIgnoredLineContent, 0, precedingUnIgnoredLineContent.length()), EnterAction.IndentAction.Indent, precedingUnIgnoredLine);
    } else if (indentRulesSupport.shouldDecrease(precedingUnIgnoredLineContent)) {
      return new AxiomEditorTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(precedingUnIgnoredLineContent, 0, precedingUnIgnoredLineContent.length()), null, precedingUnIgnoredLine);
    } else {
      ...
      if (precedingUnIgnoredLine == 0) {
        return new AxiomEditorTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(precedingUnIgnoredLine)), null, precedingUnIgnoredLine);
      }
      ...
      if (((previousLineIndentMetadata & (IndentRulesSupport.IndentConsts.INCREASE_MASK | IndentRulesSupport.IndentConsts.DECREASE_MASK)) == 0) && (previousLineIndentMetadata & IndentRulesSupport.IndentConsts.INDENT_NEXTLINE_MASK) == 0 && previousLineIndentMetadata > 0) {
        ...
        return new AxiomEditorTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(stopLine + 1)), null, stopLine + 1);
      }

      if (honorIntentialIndent) {
        return new AxiomEditorTMNewlineHandler.InheritIndentResult(
      ...
      for (var i = precedingUnIgnoredLine; i > 0; i--) {
        ...
        if (indentRulesSupport.shouldIncrease(lineContent)) {
          return new AxiomEditorTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(lineContent), EnterAction.IndentAction.Indent, i);
          ...
          return new AxiomEditorTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(lineContent), null, i);
        }
      }

      return new AxiomEditorTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(1)), null, 1);
    }
  }
   ...
   */
  public int getPrecedingValidLine(AxiomEditorTMNewlineHandler.WrapperContent content, int lineNumber) {
  ...
  /*
     ...
     */
  private static class WrapperContentImp implements AxiomEditorTMNewlineHandler.WrapperContent {

...
# Introduction
This guide provides a comprehensive introduction to creating plugins for AxiomEditor, a powerful and flexible code editor designed for Android devices. It is designed for beginners with basic Java knowledge.

## What are Plugins?

Plugins are small, independent programs that add new features or extend the functionality of an existing application. In AxiomEditor, plugins allow you to customize the editor to fit your specific needs.

## Plugin Structure and Key Components

AxiomEditor plugins are written in Java and interact with the application through defined interfaces. Here's a breakdown of the core components:
...
```java
package com.axiomeditor.plugins;
...
-   `onPluginLoaded(@NonNull PluginContext context)`: This method is called when your plugin is loaded into AxiomEditor. The `PluginContext` object provides access to AxiomEditor's functionalities.

### 2. The `PluginContext` Interface

This interface is crucial for plugin development. It provides methods to interact with AxiomEditor:
...
```java
package com.axiomeditor.plugins.editor;
...
These are used for adding items to AxiomEditor's menu:

```java
package com.axiomeditor.plugins.menu;
...
4.  **Install the Plugin:** Place the compiled `.jar` file in AxiomEditor's plugin directory.

5.  **Run AxiomEditor:** When you start AxiomEditor, your plugin should be loaded, and you should see "Hello, Plugin!" in the logs.
...
import com.axiomeditor.plugins.Plugin;
import com.axiomeditor.plugins.PluginContext;
import com.axiomeditor.plugins.menu.MenuAction;
import com.axiomeditor.plugins.menu.MenuItem;
import java.io.File;
...
1. **Open an Issue:** Go to the [GitHub repository](https://github.com/Axiom-Editor/Axiom-Editor/issues) and open a new issue detailing the API or function you need.
2. **Join the Telegram Group:** Join our [Telegram group](https://t.me/axiomeditor) and share your request with the community.
